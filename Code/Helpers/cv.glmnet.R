my.cv.glmnet <- function (x, y, weights, offset = NULL, lambda = NULL, type.measure = c("mse", 
                                                                                        "deviance", "class", "auc", "mae"), nfolds = 10, foldid, 
                          grouped = TRUE, keep = FALSE, parallel = FALSE, ...) 
{
  cat("## Using modified version of the cv.glmnet function...\n")
  ...
  if (keep) {
    coeffs <- sapply(outlist, coef)
    ttls <- lapply(lambda, 
                   function(lambda_i) do.call(cBind, lapply(outlist, coef, s = lambda_i)))
    coeffOcc <- sapply(ttls, function(coeffs) rowMeans(coeffs != 0))
    colnames(coeffOcc) <- lambda
    out = c(out, list(fit.preval = cvstuff$fit.preval, foldid = foldid, coeffOcc = coeffOcc, fullCoeffs = ttls))
  }
  lamin = if (cvname == "AUC") 
